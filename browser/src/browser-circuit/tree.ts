...
export function buildMerkleSumTree(balances: bigint[], salts: Uint8Array[]): MerkleSumTree {
  if (balances.length !== LEAF_COUNT) {
    throw new Error(`buildMerkleSumTree requires exactly ${LEAF_COUNT} balances, got ${balances.length}`);
  }
  if (salts.length !== LEAF_COUNT) {
    throw new Error(`buildMerkleSumTree requires exactly ${LEAF_COUNT} salts, got ${salts.length}`);
  }
